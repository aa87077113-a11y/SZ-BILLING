...
(function(){
  ...
  async function init(){
    ...
    if(!u){status('Not logged in');return}
    try{await refreshTables();cloudReady=true;status('Connected • '+(u.email||u.name||u.$id));await mergeFromCloud();await pushLocal();}
  ...
  window.cloudSyncNow=async function(){if(!initSdk())return toast('Cloud SDK unavailable');try{let u=await currentUser();if(!u){cloudLoginPrompt();return}await refreshTables();cloudReady=true;status('Connected • '+(u.email||u.name||u.$id));await mergeFromCloud();await pushLocal();toast('Cloud sync complete')}catch(e){console.error(e);toast(e?.message||'Cloud sync failed')}};
  window.cloudLoginPrompt=async function(){
    if(!initSdk())return toast('Cloud SDK unavailable');
    try{const active=await currentUser();if(active){cloudReady=false;await init();toast('Already logged in');return}}catch(_){}
    const email=prompt('Appwrite Email'); if(!email)return;
    const password=prompt('Appwrite Password'); if(!password)return;
    try{await account.createEmailPasswordSession({email,password});cloudReady=false;await init();toast('Cloud login successful')}catch(e){
      const active=await currentUser();
      if(active){cloudReady=false;await init();toast('Already logged in')}else{console.error(e);toast(e?.message||'Login failed')}
    }
  };
  window.cloudRegisterPrompt=async function(){
    ...
    try{await account.create({userId:Appwrite.ID.unique(),email,password,name});
      const active=await currentUser();
      if(!active)await account.createEmailPasswordSession({email,password});
      cloudReady=false;await init();toast('Account created and logged in')
    }catch(e){
      const active=await currentUser();
      if(active){cloudReady=false;await init();toast('Account created and logged in')}else{console.error(e);toast(e?.message||'Account creation failed')}
    }
  };

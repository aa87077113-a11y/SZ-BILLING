...
(function(){
  ...
  window.cloudSyncNow=async function(){
    ...
    const a=await authState();
    if(!a){status('Not logged in');cloudLoginPrompt();return}
    try{
      await refreshTables();cloudReady=true;
      const u=a.user||{};status('Connected • '+(u.email||u.name||a.session.userId||'Active session'));
      await pushLocal();await mergeFromCloud();toast('Cloud sync complete')
    }catch(e){console.error(e);cloudReady=false;status('Sync failed: '+(e?.message||'Unknown error'));toast('Cloud sync failed')}
  };
  window.cloudLoginPrompt=async function(){
    if(!initSdk())return toast('Cloud SDK unavailable');
    const existing=await authState();
    if(existing){cloudReady=false;await init();return toast('Already logged in')}
    const email=prompt('Appwrite Email'); if(!email)return;
    const password=prompt('Appwrite Password'); if(!password)return;
    try{await account.createEmailPasswordSession({email,password});cloudReady=false;await init();toast('Cloud login successful')}
    catch(e){console.error(e);const msg=e?.message||'Login failed';status('Login failed: '+msg);toast(msg)}
  };
  window.cloudRegisterPrompt=async function(){
    ...
    try{await account.create({userId:Appwrite.ID.unique(),email,password,name});await account.createEmailPasswordSession({email,password});cloudReady=false;await init();toast('Account created and logged in')}
    catch(e){console.error(e);const msg=e?.message||'Account creation failed';status('Account error: '+msg);toast(msg)}
  };

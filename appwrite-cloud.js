/* SZ Billing V50 - Appwrite cloud bridge: auth gate + diagnostics + sync. */
(function(){
  ...
  window.SZCloud={CFG};
  function status(t){const e=document.getElementById('cloudStatus');if(e)e.textContent='Cloud: '+t;const g=document.getElementById('cloudGateStatus');if(g)g.textContent=t;}
  function gate(show){const e=document.getElementById('cloudGate');if(e)e.style.display=show?'flex':'none';}
  function initSdk(){
  ...
  async function init(){
    ...
    if(!a){cloudReady=false;status('Not logged in');gate(true);return false}
    try{
      await refreshTables();
      cloudReady=true;
      const u=a.user||{};
      status('Connected • '+(u.email||u.name||a.session.userId||'Active session'));
      gate(false);
      ...
      cloudReady=false;
      status('Login OK • Cloud setup error');
      gate(false);
      toast('Login successful, but cloud sync setup failed: '+errText(e));
      return false;
    }
  }
  ...
  async function pushLocal(){
    ...
    try{
      for(const name of TABLE_NAMES){
        for(const x of rowsFor(name)) await tablesDB.upsertRow({databaseId:CFG.databaseId,tableId:tableMap[name],rowId:String(x.id),data:x.data});
      }
      status('Connected • Synced');
      return true;
    }catch(e){console.error('Cloud push failed',e);status('Connected • Sync failed');toast('Sync failed: '+errText(e));return false}
  ...
  async function mergeFromCloud(){
    ...
    for(const name of TABLE_NAMES){
      ...
      const key=name;if(!Array.isArray(db[key]))db[key]=[];const map=new Map(db[key].map(x=>[x.id,x]));incoming.forEach(x=>map.set(x.id,x));db[key]=Array.from(map.values());
    }
  ...
  window.cloudSyncNow=async function(){
    ...
    if(!u){status('Not logged in');gate(true);return toast('Please login to Cloud first')}
    try{await refreshTables();cloudReady=true;gate(false);const ok=await pushLocal();if(ok){await mergeFromCloud();toast('Cloud sync complete')}}catch(e){console.error('Sync Now failed',e);status('Connected • Sync failed');toast('Cloud sync failed: '+errText(e))}
  };
  ...
  window.cloudRegisterPrompt=async function(){
    ...
    try{await account.create({userId:Appwrite.ID.unique(),email,password,name});await account.createEmailPasswordSession({email,password});cloudReady=false;const ok=await init();toast(ok?'Account created and logged in':'Account created, but sync setup needs attention')}catch(e){console.error(e);toast(errText(e)||'Account creation failed')}
  };
  window.cloudLogout=async function(){
    if(!initSdk())return;
    try{await account.deleteSession({sessionId:'current'});cloudReady=false;status('Not logged in');gate(true);toast('Logged out')}catch(e){toast(errText(e)||'Logout failed')}
  };
})();

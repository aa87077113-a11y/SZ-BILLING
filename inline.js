...
async function shareInvoiceAs(kind){
  ...
  let file;try{file=new File([blob],fileName,{type:'application/pdf'});}catch(e){}

  // Direct native file share: do not run save(), timers, or navigation before this call.
  if(file && typeof navigator.share==='function'){
    try{
      await navigator.share({files:[file],title:`${title} ${obj.no}`});
      ...
      // A few Android builds require text + file together.
      try{
  ...
  // Reliable fallback: download the exact PDF, then open the customer's WhatsApp chat.
  // wa.me itself cannot receive a local file attachment from browser JavaScript.
  try{
    const url=URL.createObjectURL(blob),a=document.createElement('a');
    a.href=url;a.download=fileName;document.body.appendChild(a);a.click();a.remove();
    obj.shareAs=kind;db.history.push({type:title+' WhatsApp PDF',customer:cu.name,amount:c.total,date:new Date().toLocaleString()});save();
    setTimeout(()=>{URL.revokeObjectURL(url);window.location.href='https://wa.me/'+String(cu.wa).replace(/\D/g,'')+'?text='+encodeURIComponent(msg);},300);
  }catch(e){toast('PDF WhatsApp sharing is not supported on this device');}
}
...
function pendingPost(){
  ...
  canvas.toBlob(blob=>{
    ...
    if(file&&typeof navigator.share==='function'){
      navigator.share({title:'Payment Reminder',files:[file]}).then(()=>{db.history.push({type:'Pending Post',customer:cu.name,amount:pending,date:new Date().toLocaleString()});save()}).catch(e=>{if(!e||e.name!=='AbortError')toast('Share cancelled')});return;
    }
    ...
    setTimeout(()=>{URL.revokeObjectURL(url);window.location.href='https://wa.me/'+String(cu.wa).replace(/\D/g,'')},500);

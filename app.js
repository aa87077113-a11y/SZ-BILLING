...
function invoiceWA(){
  if(!iEdit)return toast('Open Invoice first');
  let cu=db.customers.find(x=>x.id===iEdit.customerId);
  if(!cu)return toast('Select customer');
  if(!cu.wa)return toast('Customer WhatsApp number missing');
  shareInvoiceAs(iEdit.shareAs==='quotation'?'quotation':'invoice');
}
...
async function shareInvoiceAs(kind){
  ...
  const phone=String(cu.wa||'').replace(/\D/g,'');if(!phone)return toast('Customer WhatsApp number missing');
  const title=kind==='quotation'?'Quotation':'Invoice';
  const c=calc(obj),paid=Number(obj.paid||0),pending=Math.max(0,c.total-paid);
  const msg=`Assalam-o-Alaikum ${cu.name},\n\nPlease find your ${title.toLowerCase()} ${obj.no}.\nTotal: ${money(c.total)}${kind==='invoice'?`\nReceived: ${money(paid)}\nPending: ${money(pending)}`:''}\n\nThank you for choosing ${db.settings.name||'SZ Billing'}.`;
  const blob=makeInvoicePdf(obj,kind),fileName=`${title}-${obj.no}.pdf`;
  // Keep the PDF as a real file. Browser/PWA WhatsApp URLs cannot attach local files themselves.
  try{const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=fileName;a.rel='noopener';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000)}catch(e){toast('PDF could not be prepared');return}
  try{
    const file=new File([blob],fileName,{type:'application/pdf'});
    // Best available web path: share the actual PDF file. WhatsApp is then selectable as the share target.
    if(typeof navigator.share==='function'){
      const fileShareOk=!navigator.canShare || navigator.canShare({files:[file]});
      if(fileShareOk){
        await navigator.share({title:`${title} ${obj.no}`,text:msg,files:[file]});
        obj.shareAs=kind;db.history.push({type:title+' WhatsApp PDF',customer:cu.name,amount:c.total,date:new Date().toLocaleString()});save();return;
      }
    }
  }catch(e){if(e&&e.name==='AbortError')return}
  // Fallback: open the exact customer's WhatsApp chat, with PDF already saved for attachment.
  obj.shareAs=kind;db.history.push({type:title+' WhatsApp PDF',customer:cu.name,amount:c.total,date:new Date().toLocaleString()});save();
  const waUrl='https://wa.me/'+phone+'?text='+encodeURIComponent(msg+'\n\nPDF: '+fileName);
  try{window.location.href=waUrl}catch(e){window.open(waUrl,'_blank','noopener')}
}
function renderWA(){let s=document.getElementById('waCustomer');if(!s)return;s.innerHTML=customerOptions(window.waCustomer||'');if(!window.waCustomer&&db.customers[0]){window.waCustomer=db.customers[0].id;s.value=window.waCustomer}waLoad();let h=document.getElementById('waHistory');if(h)h.innerHTML=db.history.filter(x=>String(x.type||'').includes('WhatsApp')).slice(-10).reverse().map(x=>`<div class="item"><div><b>${esc(x.type)}</b><div class="muted small">${esc(x.customer)} • ${esc(x.date)}</div></div><b>${money(x.amount)}</b></div>`).join('')||'<div class="empty">No WhatsApp history.</div>'}
...
function pendingPost(){
  ...
  g.fillStyle='#0b1f3a';g.fillRect(0,0,1200,300);
  g.fillStyle='#1d70d6';g.fillRect(0,292,1200,8);
  ...
  g.fillStyle='#1d70d6';g.beginPath();g.arc(125,437,42,0,Math.PI*2);g.fill();
  g.fillStyle='#ffffff';g.font='bold 30px Arial';g.textAlign='center';g.fillText(String(cu.name||'C').trim().charAt(0).toUpperCase(),125,447);g.textAlign='left';
  g.fillStyle='#718096';g.font='16px Arial';g.fillText('ACCOUNT HOLDER',195,408);
  g.fillStyle='#0b1f3a';g.font='bold 31px Arial';g.fillText(String(cu.name||'Valued Customer').slice(0,32),195,450);
  g.fillStyle='#718096';g.font='18px Arial';g.fillText('Invoice #'+String(inv.no||''),195,485);
  // Outstanding amount card
  g.fillStyle='#f3f8ff';g.beginPath();g.roundRect(60,565,1080,270,26);g.fill();g.strokeStyle='#cfe0f4';g.stroke();
  g.fillStyle='#52657b';g.font='bold 17px Arial';g.fillText('OUTSTANDING BALANCE',100,625);
  g.fillStyle='#0b1f3a';g.font='bold 67px Arial';g.fillText(money(pending),100,715);
  g.fillStyle='#718096';g.font='18px Arial';g.fillText('Amount currently due',100,755);
  g.fillStyle='#d8e3ef';g.fillRect(600,605,1,175);
  g.fillStyle='#66788e';g.font='17px Arial';g.fillText('TOTAL BILL',655,635);g.fillStyle='#0b1f3a';g.font='bold 26px Arial';g.fillText(money(c.total),655,680);
  g.fillStyle='#66788e';g.font='17px Arial';g.fillText('RECEIVED',655,725);g.fillStyle='#0b1f3a';g.font='bold 26px Arial';g.fillText(money(paid),655,770);
  if(inv.due){g.fillStyle='#1d70d6';g.font='bold 17px Arial';g.fillText('DUE DATE  '+inv.due,100,805)}
  // Message section
  g.fillStyle='#0b1f3a';g.font='bold 27px Arial';g.fillText('Dear Customer,',75,910);
  g.fillStyle='#3f5065';g.font='21px Arial';
  ['We appreciate your trust and continued business.','When convenient, please arrange the outstanding payment.','If payment has already been made, kindly share the confirmation','so our accounts record can be updated. Thank you.'].forEach((t,i)=>g.fillText(t,75,955+i*38));
  // Footer / contact strip
  g.fillStyle='#0b1f3a';g.beginPath();g.roundRect(60,1165,1080,220,24);g.fill();
  g.fillStyle='#ffffff';g.font='bold 26px Arial';g.fillText(String(db.settings.name||'SZ BILLING').slice(0,36),100,1225);
  g.fillStyle='#b8c9dc';g.font='18px Arial';g.fillText('For payment confirmation or account assistance',100,1262);
  if(db.settings.phone){g.fillStyle='#ffffff';g.font='bold 20px Arial';g.fillText('Contact  '+String(db.settings.phone).slice(0,42),100,1310)}
  if(db.settings.address){g.fillStyle='#b8c9dc';g.font='17px Arial';g.fillText(String(db.settings.address).slice(0,78),100,1347)}
  g.fillStyle='#7f93aa';g.font='15px Arial';g.fillText('This notice is a friendly courtesy and not a demand notice.',75,1435);
  canvas.toBlob(blob=>{
    if(!blob)return toast('Could not create post');
    let file;try{file=new File([blob],'Payment-Pending-'+inv.no+'.png',{type:'image/png'})}catch(e){}
    // Save the actual image first, then open this customer's WhatsApp chat directly.
    // wa.me cannot attach a local image from a browser/PWA automatically.
    let url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=file?.name||('Payment-Pending-'+inv.no+'.png');document.body.appendChild(a);a.click();a.remove();
    db.history.push({type:'Pending Post',customer:cu.name,amount:pending,date:new Date().toLocaleString()});save();
    setTimeout(()=>{URL.revokeObjectURL(url);window.location.href='https://wa.me/'+String(cu.wa).replace(/\D/g,'')},700);
  },'image/png');
}
...
function renderBillingEditor(){
  ...
  <div class="actions"><button class="btn" onclick="saveBilling()">Save Invoice</button><button class="btn ok" onclick="markFullPaidAndSave()">✓ Payment Full Paid</button><button class="btn secondary" onclick="viewCurrentBilling()">View</button><button class="btn accent" onclick="printDoc('invoice')">Print / PDF</button><button class="btn ok" onclick="invoiceWA()">WhatsApp</button><button class="btn secondary" onclick="pendingPost()">Pending Post</button></div>
  </div>`;
}
...
function markSavedInvoicePaid(id){
  ...
  inv.paid=Number(inv.total||0);inv.pending=0;
  db.history.push({type:'Invoice Fully Paid',customer:inv.customer||'',amount:inv.total||0,date:new Date().toLocaleString()});
  save();toast('Invoice marked as Fully Paid');renderBilling();renderDashboard();
}
